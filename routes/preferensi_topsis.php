...
$preferensi = [];
foreach ($alternatif as $id_alternatif => $nama_alternatif) {
    $D_plus = $jarakIdealPositif[$id_alternatif];
    $D_minus = $jarakIdealNegatif[$id_alternatif];

    // Menghindari pembagian dengan nol
    if ($D_plus + $D_minus > 0) {
        $preferensi[$id_alternatif] = $D_minus / ($D_plus + $D_minus);
    } else {
        $preferensi[$id_alternatif] = 0; // Atur preferensi ke 0 jika kedua jarak nol
    }
}

// Urutkan preferensi dari yang tertinggi
arsort($preferensi);
...
<body>
    ...
    <div class="card shadow">
        <div class="table-responsive">
            <div class="card-header py-3" style="text-align: center; background-color: #167395; color: white; font-weight:bold">Rekapitulasi Jarak Solusi Ideal Positif dan Negatif</div>
            <div class="card-body">
                <div class="table-responsive">
                    <?php
                    echo "
            ...
            <td>" . round($jarakIdealPositif[$id_alternatif], 4) . "</td>
            <td>" . round($jarakIdealNegatif[$id_alternatif], 4) . "</td>
            <td>" . round($preferensi[$id_alternatif], 4) . "</td>
          </tr>";
                    }
                    echo "</table>"
                    ?>
    ...
    <div class="card shadow">
        <div class="card-header py-3" style="text-align: center; background-color: #167395; color: white; font-weight:bold">Nilai Preferensi Alternatif</div>
        <div class="card-body">
            <div class="table-responsive">
                <?php
                echo "<table border='1'>
                ...
                foreach ($preferensi as $id_alternatif => $nilai_preferensi) {
                    echo "<tr>
                        ...
                        <td>" . round($nilai_preferensi, 4) . "</td>
                      </tr>";
                }
                echo "</table>";
                ?>
